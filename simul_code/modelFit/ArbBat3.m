function [SBJ]=ArbBat3(maxi,mode, PE, MODELS,PreBehav, PreBlck,MainBehav,MainBlck,list_sbj,param_init)
ttt = clock;
rng(floor(ttt(6)*1000));
for ppi = 1 : 4
    rng(floor(ttt(6)*1000));
...
%     data_pre=load([pwd '/result_save/' list_sbj{i} '_pre_1.mat']);
    ...
    % max sess eval
    % main save
    temp = [];    tt = dir([pwd '/2016_depression data/' list_sbj{i} '/']);
    tt = {tt.name};
    maxsess = length(cell2mat(strfind(tt,['_fmri_']))) - 1;
    for ii = 1 : maxsess
