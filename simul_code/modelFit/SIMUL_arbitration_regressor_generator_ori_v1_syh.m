...
path0=pwd;
path0=[pwd '\2016_depression data'];
...
list_sbj_included=[1,3,4,11,12,18,19,20,24,25,27,31:1:41];
...
for i=1:1:num_sbj_included %=1. process only 1 subject
    ...
    file_name_full=[mode.path_ext '\' LIST_SBJ{1,ind_sbj_included(i)} file_name];
    filefile = dir(file_name_full);
    load([mode.path_ext '\' LIST_SBJ{1,ind_sbj_included(i)} '\' filefile.name]);
...
if option_best_param_load == 1
    ...
    for i = 1:1:num_sbj_included
        SBJ_list = dir([seed_path_result '\SBJ_' LIST_SBJ_included{i} '_DPON_*']);
        sim_tot = length(SBJ_list);
        minmin=999; % meaningless large number
        for is = 1 : 1 : sim_tot
            sbsb = load([seed_path_result '/' SBJ_list(is).name]);
...
mode.param_BoundL = [0.4 0.02 0.5 0.5 0.01 0.01];
mode.param_BoundU = [0.7 0.35 10 10 0.5 0.3];
...
if(option_optimizing_model==0)
    for ind_sbj=1:1:size(SBJ2,2)
        ...
        mode.out=1;
        myFunc_bu = @(x) eval_ArbitrationRL3c(x, SBJ_test, mode); % define a new anonymous function
        ...
        mode.out=99;
        SBJ_test=eval_ArbitrationRL3c(model_BayesArb.param,SBJ_test,mode);
        % [3] Save
...
%                         ind_reg_abs=ind_regressor_type_dummy.abs_pos_in_design_mat(nn);      list_name_for_contrast{1,ind_reg_abs}=pmod(1,ind_reg).name{1,ind_reg_param};
            ...
            if(Is_save_files_local==1)
                ...
                save([save_for_SPM '\' save_file_name],'durations','names','onsets', 'pmod' );
            end
...
%                 eval(['save ' save_path_neuroecon save_file_name ' durations names onsets pmod'])
            ...
            if(Is_save_files_local==1)
                ...
                save([save_for_SPM '\' save_file_name],'durations','names','onsets', 'pmod','ind_regressor_type_dummy' );
            end
            if(Is_save_files_cluster==1)
%                 eval(['save ' save_path_neuroercon save_file_name ' durations names onsets pmod ind_regressor_type_dummy'])
            end
    ...
    if(Is_save_files_local==1)
        ...
        save([save_for_SPM '\contrast_spm.mat'],'contrast_spm', 'ind_regressor_type_base', 'ind_regressor_type_dummy', 'param_regressor_type_cue_abs_pos_in_design_mat', 'list_name_for_contrast' );
    end
    if(Is_save_files_cluster==1)
