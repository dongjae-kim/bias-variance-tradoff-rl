function [myArbitrator myState1 myState2]=BEBP_Arbitration_model(myArbitrator, myState1, myState2, myMap)
% MAIN DESCRIPTION
% 
...
% (2.5) update current ZPE(zero prediction error) cluster using whole
% PE_history and update m1_wgt for indicate ZPE cluster.
myArbitrator.m1_cluster_hist = [myArbitrator.m1_cluster_hist , [double(ind_update) ; myState1.SPE_history(myState1.index)]];
m1_mean = zeros(1,max(uint16(myArbitrator.m1_cluster_hist(1,:))));
for cl = min(uint16(myArbitrator.m1_cluster_hist(1,:))) : max(uint16(myArbitrator.m1_cluster_hist(1,:)))
    cl_ind = find(uint16(myArbitrator.m1_cluster_hist(1,:)==cl));
    if cl_ind
        m1_mean(1,cl) = mean(myArbitrator.m1_cluster_hist(2,cl_ind));
        if m1_mean(1,cl) == 0
            m1_mean(1,cl) = 1E-7;
        end
    end
end
mama=zeros(2,size(find(m1_mean~=0),2));
mama(1,:)=find(m1_mean~=0);
mama(2,:)=m1_mean(find(m1_mean~=0));
[~, myArbitrator.PE_index_m1] = sort(abs(mama(2,:)));
myArbitrator.PE_index_m1 = mama(1, myArbitrator.PE_index_m1);
myArbitrator.m1_thre_zpe = m1_mean(myArbitrator.PE_index_m1(1));
myArbitrator.m1_wgt(myArbitrator.PE_index_m1(1)) = 1;
...
% (2.5) update current ZPE(zero prediction error) cluster using whole
% PE_history and update m2_wgt for indicate ZPE cluster.
myArbitrator.m2_cluster_hist = [myArbitrator.m2_cluster_hist , [double(ind_update) ; myState2.RPE_history(myState2.index)]];
m2_mean = zeros(1,max(uint16(myArbitrator.m2_cluster_hist(1,:))));
for cl = min(uint16(myArbitrator.m2_cluster_hist(1,:))) : max(uint16(myArbitrator.m2_cluster_hist(1,:)))
    cl_ind = find(uint16(myArbitrator.m2_cluster_hist(1,:)==cl));
    if cl_ind
        m2_mean(1,cl) = mean(myArbitrator.m2_cluster_hist(2,cl_ind));
        if m2_mean(1,cl) == 0
            m2_mean(1,cl) = 1E-7;
        end
    end
end
mama=zeros(2,size(find(m2_mean~=0),2));
mama(1,:)=find(m2_mean~=0);
mama(2,:)=m2_mean(find(m2_mean~=0));
[~, myArbitrator.PE_index_m2] = sort(abs(mama(2,:)));
myArbitrator.PE_index_m2 = mama(1, myArbitrator.PE_index_m2);
myArbitrator.m2_thre_zpe = m2_mean(myArbitrator.PE_index_m2(1));
myArbitrator.m2_wgt(myArbitrator.PE_index_m2(1)) = 1;
