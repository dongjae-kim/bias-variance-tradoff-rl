function [myArbitrator]=BEBP_Arbitration_model_Init(N_state,N_action,N_transition, myMap, data_in)
...
% prior knowledge to compare about the cluster
 [~, myArbitrator.PE_index_m1] = sort(abs(m1_mean));
 [~, myArbitrator.PE_index_m2] = sort(abs(m2_mean));
 
% Threshold for ZPE cluster, lowest mean
myArbitrator.m1_thre_zpe =0;
myArbitrator.m2_thre_zpe =0;
...
myArbitrator.m1_cluster_hist = []; myArbitrator.m2_cluster_hist = []; 
myArbitrator.m1_PE_full_hist = []; myArbitrator.m2_PE_full_hist = [];
...
myArbitrator.m1_wgt=zeros(myArbitrator.K1,1); % myArbitrator.m1_wgt(myArbitrator.PE_index_m1(1)) = 1; for the model using global cluster mean.
myArbitrator.m2_wgt=zeros(myArbitrator.K2,1); % myArbitrator.m2_wgt(myArbitrator.PE_index_m2(1)) = 1; for the model using global cluster mean. 
