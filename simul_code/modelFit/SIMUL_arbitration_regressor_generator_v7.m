...
LIST_REGRESSOR={'SPE', 'RPE', 'uncertaintyM1', 'uncertaintyM2', 'meanM1', 'meanM2', 'invFanoM1', 'invFanoM2', 'weigtM1', 'weigtM2', 'Qfwd', 'Qsarsa', 'Qarb', 'dQbwdEnergy', 'dQbwdMean',...
    ...
    'invFanoM1_meancorrected','invFanoM2_meancorrected','Prob_actionR'};
TYPE_REGRESSOR=[1 1, 1.5 1.5, 1.5 1.5, 1.5 1.5, 2 2, 1.5 1.5 1.5 1.5 1.5 1.5 1.5 1.5 1.5 1.5 1.5 1.5 1.5 1.5 1.5 1.5 -99]; % 1: parametric modulation (0-duration), 1.5:parmetric modulation (non-zero duration), 1.7:parametric modulation (with decision onset)  2: extra continuous parametric modulation (TR-fixed) - this will be used by "dummy" regressor. -99: regressor only for analysis
row_mat=[7 7 8 8 8 8 8 8 7 7 7 7 7 7 7 8 8 7 7 8 8 8 8 7 8 8 7]; % from which row in the SBJ{}.regressor matrix the signal needs to be extracted. e.g., uncertainty of 0 prediction error, 
...
% [NOTE] BEST FullBayes: 'SBJ_structure_each_exp_BESTcollection_FullBayes.mat', BEST BayesArb: 'SBJ_structure_each_exp_BESTcollectionJan29.mat'
% [NOTE!!!] FullBayes: "eval_ArbitrationRL3.m">>"Bayesian_Arbitration_v3()" 
% [NOTE!!!] BayesArb: "eval_ArbitrationRL3.m">>"Bayesian_Arbitration_v4()" 
name_paramfile_to_test='SBJ_structure_each_exp_BESTcollectionJan29.mat'; % valid only if option_optimizing_model=2.
% name_paramfile_to_test='SBJ_structure_each_exp_BESTcollection_Daw'; % Daw's model
...
% [CAUTION] 'weigtM1' (TYPE_REGRESSOR=2) should be placed in the last row of
% 'param_regressor_type_cue'. If you don't want to test it, set
% DO_USE_RAND_DUMMY_REGRESSOR=1, instead of removing it from the 'param_regressor_type_cue';

% Daw's version test
% param_regressor_type_cue={'RPE', 'SPE', 'MAXinvFano12', 'Qfwd','Qsarsa','Qarb', 'dQbwdEnergy', 'weigtM1'};

% full test
param_regressor_type_cue={'SPE', 'RPE', 'uncertaintyM1','MAXinvFano12','dinvFano12','weigtM1'};

% value test
% param_regressor_type_cue={'SPE', 'RPE', 'invFanoM1','Qfwd','Qsarsa','Qarb','dQbwdEnergy', 'weigtM1'};

reg_type_go_first=[1 1.5 2]; %[1 1.5 2] [CAUTION] The order should match with 'param_regressor_type_cue'.   [CAUTION] type"2" should go always last!!!
...
DO_USE_RT_REGRESSOR=1;
DO_USE_RAND_DUMMY_REGRESSOR=1; % use a random noise - to disable the dummy
...
Do_behavioral_analysis=[1]; % [NOTE] use list_sbj_included=[2:1:14 16:1:19 21 23:1:24] for better results
...
if(option_optimizing_model==2) % [NOTE] replace initial SBJ = just read SBJ from the "SBJ_structure.mat"
    ...
    % backup plan
    load_file_name=['SBJ_structure_backup_batch_Oct5.mat']; % DO NOT CHANGE THIS. THIS WILL NOT USED.
    eval(['load ' save_path_result load_file_name])    
    SBJ_backup_plan=SBJ;
    clear SBJ
    % 
...
%         if(isfield(SBJ0{1,1}.model_BayesArb.mode, 'boundary_12')==1) % parameter length=6 case
        ...
        if(length(SBJ0{1,1}.model_BayesArb.param)==8) % parameter length=8 case
...
%     bar(out_corr_invF_1_2);
    ...
    all_RT_each_cond=[]; 
    all_corr_RT_PMB_pval=[];    all_corr_RT_PMB_RR=[];
    for jj2=1:1:size(SBJ,2)        % each subject
...
%         end
        ...
        % 00-2. RT vs P_MB
        all_RT_00=[]; all_PMB=[]; ind_reg_00=1;
        val_RT_totalcnt_each_cond=zeros(1,4);    val_RT_each_cond=zeros(1,4);        % G, G', H, H'
        %         if(SBJ{1,jj2}.map_type==2) % new map only
        nn_session=size(SBJ{1,jj2}.HIST_block_condition,2);
        for ff2=1:1:nn_session
            nn_trial=size(SBJ{1,jj2}.HIST_block_condition{1,ff2},2);
            for qq2=1:1:nn_trial
                all_RT_00=[all_RT_00 SBJ{1,jj2}.HIST_behavior_info{1,ff2}(qq2,9:10)];
                all_PMB=[all_PMB SBJ{1,jj2}.regressor{1,24}.value(7,ind_reg_00:ind_reg_00+1)];
                ind_reg_00=ind_reg_00+3;
            end
        end
        %         end
        [r00,p00]=corrcoef(all_PMB,all_RT_00);
        all_corr_RT_PMB_pval=[all_corr_RT_PMB_pval p00(1,2)];
        all_corr_RT_PMB_RR=[all_corr_RT_PMB_RR r00(1,2)];
...
%     errY1 = zeros(2,2,2);
    ...
    current_axis=axis;
%     axis([current_axis(1:2) 0 1]);
    [h p]=ttest(all_RT_each_cond(:,3),all_RT_each_cond(:,4));%ttest
    [h p]=ttest((all_RT_each_cond(:,1)+all_RT_each_cond(:,2))/2,(all_RT_each_cond(:,3)+all_RT_each_cond(:,4))/2);%ttest
    
    % correlation btw RT and P_MB @#$
%     figure('Name','Corr btw RT and P(MB)');
%     plot(all_PMB,all_RT_00,'o');


    
    %% 01. how well subjects are doing in the task? - model's choice value vs subjects' actual choices
    act_sbj_all=cell(1,size(SBJ,2));    act_prob_model_all=cell(1,size(SBJ,2));
    proportion_Rchoice_model_predicted=[];
    % bin set : % row for the lower/upper boudn of each bin
    num_bin=5;
    bin_set_prob=[];
    for kk=1:1:num_bin
        bin_set_prob=[bin_set_prob; [(kk-1)/num_bin kk/num_bin]];
    end
    for jj2=1:1:size(SBJ,2)        % each subject
        nn_session=size(SBJ{1,jj2}.HIST_block_condition,2); act_sbj=[];
        % load sbjs' action choice
        for ff2=1:1:nn_session
            nn_trial=size(SBJ{1,jj2}.HIST_block_condition{1,ff2},2);
            for qq2=1:1:nn_trial
                act_sbj=[act_sbj SBJ{1,jj2}.HIST_behavior_info{1,ff2}(qq2,[7 8])]; % concatenate sbjs' actions
            end
        end        
        act_sbj_all{1,jj2}=act_sbj - 1; % 0-left, 1-right
        % load model's R action choice prob : 0-left, 1-right
        [ind_tmp]=find((SBJ{1,jj2}.regressor{1,27}.value(4,:)==1)|(SBJ{1,jj2}.regressor{1,27}.value(4,:)==2));
        act_prob_model{1,jj2}=SBJ{1,jj2}.regressor{1,27}.value(7,ind_tmp);
        % bin
        ind_tmp_binR=cell(1,size(bin_set_prob,1));  proportion_Rchoice=[];
        for jj3=[1:1:size(bin_set_prob,1)]
            ind_tmp_binR{1,jj3}=find((act_prob_model{1,jj2}>=bin_set_prob(jj3,1))&(act_prob_model{1,jj2}<bin_set_prob(jj3,2)));
            proportion_Rchoice=[proportion_Rchoice length(find(act_sbj_all{1,jj2}(ind_tmp_binR{1,jj3})==1))/length(act_sbj_all{1,jj2}(ind_tmp_binR{1,jj3}))]; % proportion of sbjs' R-choice
        end      
        proportion_Rchoice_model_predicted=[proportion_Rchoice_model_predicted; proportion_Rchoice];        
    end
    % if there is empty bin (NaN), then replace it with median value
    aa=isnan(proportion_Rchoice_model_predicted);
    for jj4=1:1:size(aa,2)
        bb=find(aa(:,jj4)==1);
        if(length(bb)>0)
            vvv=median(proportion_Rchoice_model_predicted(find(aa(:,jj4)==0),jj4));
            proportion_Rchoice_model_predicted(bb,jj4)=vvv;
        end
    end
    % (2) scatter
    figure('Name','proportion of R-choice (scatter)');  in=ones(size(proportion_Rchoice_model_predicted,1),1)*[1:1:size(proportion_Rchoice_model_predicted,2)];
    hold on
    xx=reshape(in,[1 size(proportion_Rchoice_model_predicted,1)*size(proportion_Rchoice_model_predicted,2)]);
    yy=reshape(proportion_Rchoice_model_predicted,[1 size(proportion_Rchoice_model_predicted,1)*size(proportion_Rchoice_model_predicted,2)]);
    scatter(xx,yy,5,0.5*[1 1 1],'+');
    xx0=xx/size(proportion_Rchoice_model_predicted,2);
    [poly_f] = polyfit(xx0,yy,1); 
    val_f = polyval(poly_f,xx0);
    ...
    [poly_f2] = polyfit(xx,yy,1);     val_f_sorted = polyval(poly_f2,sort(xx));    plot(sort(xx),val_f_sorted,'k');
    [coeff_r,coeff_p] = corrcoef(xx0,yy);
    errY1=[std(proportion_Rchoice_model_predicted)/sqrt(size(proportion_Rchoice_model_predicted,1))];
%     errorbar([1:1:size(proportion_Rchoice_model_predicted,2)],mean(proportion_Rchoice_model_predicted),errY1,'r')
    errorbar(mean(proportion_Rchoice_model_predicted),errY1,'xr')
    axis([0.5 size(proportion_Rchoice_model_predicted,2)+0.5 0 1])
    ylabel('Proportion of R-choice')
    hold off
    % (1) error bar
    figure('Name','proportion of R-choice');
    colormap(gray)    
    y_plot1=[mean(proportion_Rchoice_model_predicted)]; % (2 groups of 2 parameters)
    errY1=[std(proportion_Rchoice_model_predicted)/sqrt(size(proportion_Rchoice_model_predicted,1))]; % SEM: error
    hold on    
    barwitherr(errY1, y_plot1);    % Plot with errorbars
    for qq1=1:1:size(proportion_Rchoice_model_predicted,2)-1
        [ttest2_pass(qq1) b]=ttest2(proportion_Rchoice_model_predicted(:,qq1),proportion_Rchoice_model_predicted(:,qq1+1));
    end    
    hold off
    ylabel('Proportion of R-choice')
...
%         legend(list_legend0)
    ...
    minimum_num_sample_for_analysis=20;
...
%             disp(sprintf('[sbj-%02d/%02d] obtainning the threshold from the regressor vector (%02d/%02d)',jj2, size(SBJ,2),rr2,length(list_r_ind)));
    ...
    for rr2=1:1:length(list_r_ind) % each regressor
        ...
        all_consistency_r_sbj_ind=cell(length(all_r_threshold_use{jj2,rr2})-1,5);
        ...
        for jj2=[13:1:19 21 22]% subjects on the map type2
            ...
            for kk2=1:1:num_sess % each session
                ...
                action_set_prev=zeros(4,5); % #goal condition x #state
                ...
                for ii2=[1:1:size(mat_work0,1)] % each trial
                    for aa2=1:1:2 % 1st/2nd decision state
                        ...
                        val_r0=SBJ{1,jj2}.regressor{1,list_r_ind(rr2)}.value(list_r_test_row(list_r_ind(rr2)),mycol); % P(M1)
                        mycol1=find((all_r_threshold_use{jj2,rr2}-val_r0)<=0); % see if it predicts MB or MF
                        i_c=mycol1(end); % my condition = my bin#
                        if(i_c==length(all_r_threshold_use{jj2,rr2}))                                i_c=i_c-1;                            end % the biggest value                       
                       
                        % test goal block VS flexible block (simply overwrite i_c)
                        if(0)
                            switch mat_work0(ii2,18)
                                case -1
                                    i_c=1;
                                case 6
                                    i_c=2;
                        ...
                        if(action_set_prev(i_goal_condi,i_s)~=0) % if not the first visit
                            if(action_set_prev(i_goal_condi,i_s)==i_a) % if the choice in the state@ at the goal condition$ is the same as the previous one
                                all_beh_perc0(i_c,i_s)=all_beh_perc0(i_c,i_s)+1;
                            end
                            all_beh_perc0_cnt(i_c,i_s)=all_beh_perc0_cnt(i_c,i_s)+1;
                        end
                        action_set_prev(i_goal_condi,i_s)=i_a;
                    end
                end
                
                % compute choice consistency within each block set
                %                     [r_valid c_valid]=find(all_beh_perc0_cnt>=minimum_num_sample_for_analysis); %  do not include the cell whose # of sample <threshold
                [r_valid c_valid]=find(all_beh_perc0_cnt>=10); %  do not include the cell whose # of sample <threshold
                tmp1=all_beh_perc0./max(all_beh_perc0_cnt,1); % range [0,1]
                tmp2=tmp1;%abs(tmp1-0.5)*2; % range [0,1] : 0 means inconsistent and 1 means highly consistent.
                for tt2=1:1:length(r_valid) % collect choice consistency value
                    ...
                    all_consistency_r_sbj_ind{r_valid(tt2),c_valid(tt2)}=[all_consistency_r_sbj_ind{r_valid(tt2),c_valid(tt2)} jj2];                    
                end                
            end
...
%      end
     for rr2=1:1:length(ind_r_disp) % each regressor
         ...
         if(show_major_state_only==1)
             ind_valid_state=[1 4]; % show only major state
         end
         ...
         % values by enabling the part "% test goal block VS flexible block (simply overwrite i_c)"
         mat_disp_binned_goal_condi= [[0.9177    0.8508];   [0.8951    0.8198]];
         err_y_binned_goal_condi = [[0.0179    0.0367];     [0.0151    0.0289]];
         figure('Name',['Choice consistency (state 1) - model vs goal condition binning']);
         mat_disp2=[mat_disp(:,1) mat_disp_binned_goal_condi(:,1)];
         err_y2=[err_y(:,1) err_y_binned_goal_condi(:,1)];
         barwitherr(err_y2', mat_disp2');          
         figure('Name',['Choice consistency (state 4) - model vs goal condition binning']);
         mat_disp2=[mat_disp(:,2) mat_disp_binned_goal_condi(:,2)];
         err_y2=[err_y(:,2) err_y_binned_goal_condi(:,2)];
         barwitherr(err_y2', mat_disp2');          
...
%                         ind_reg_abs=ind_regressor_type_dummy.abs_pos_in_design_mat(nn);      list_name_for_contrast{1,ind_reg_abs}=pmod(1,ind_reg).name{1,ind_reg_param};
            ...
            %% Reaction time regressor  - 2013, June27
            % [RT-duration]
            
            if(DO_USE_RT_REGRESSOR==1)
                use_model_regressor_cue=1;
                ...
                ind_reg=ind_reg+1;
                onsets{1,ind_reg}=onset_mat;
                names{1,ind_reg}=['Decision_{1T}'];  durations{1,ind_reg}=RT_mat;
                
                ind_reg_abs=ind_reg_abs+1;      list_name_for_contrast{1,ind_reg_abs}=names{1,ind_reg}; % add to the global list of regresssors
                
                % (2) pmod:
                if(use_model_regressor_cue==1)
                    name_RTRT='ReactionTime';
                    pmod(1,ind_reg).name{1,1}=name_RTRT;
                    pmod(1,ind_reg).poly{1,1}=1;
                    pmod(1,ind_reg).param{1,1}=RT_mat;
                    ind_reg_abs=ind_reg_abs+1;      list_name_for_contrast{1,ind_reg_abs}=name_RTRT; % add to the global list of regresssors                    
                end
            ...
            for nn=1:1:length(ind_regressor_type_dummy.ind_reg)
                ...
                if(DO_USE_RAND_DUMMY_REGRESSOR==1)
                    pmod(1,ind_reg).param{1,nn}=rand(1,length(regressor_extra(nn,:)));
                else
