function [data_out] = eval_ArbitrationRL5(param_in, data_in, mode)
...
%% FILL THIS
% 여기에 니중에 Artibrator 함수로부터 받아야하는 파라미터도 몇개 정의하겠음.
myArbitrator=nazonazo_init(N_state,N_action,N_transition);
% 파라미터
...
% myArbitrator.m2_absPEestimate_lr=param_in(pop_id,5); 
...
% arbitrator functioning mode % 둘다 필요한지 잘 모르겠음.필요한가? 일단 넘겨두자.
...
for ind_dev=[1:1:length(pt_devaluation)]
    ...
    for ll=1:1:tot_num_sbj % each subject
        ...
        for kk=1:1:mode.total_simul
            ...
            % end
            % 이 위에 if문 지워도 될 듯. 지금은 필요하지만 나중에는 필요 없을거 같음.
            ...
            if(mode.experience_sbj_events(1)~=-1)
                ...
                while ((i<num_max_trial0)&&(cond))
                    ...
                    while (~state_sarsa.JobComplete)
                        ...
                        state_sarsa=Model_RL(state_sarsa, param_sarsa0, map0_s, 'decision_hypo'); % 이거 때문에 action valid 넣어 줬던 거 같은 데 나중에 
                        % 1. sarsa model upate
...
%             param_fwd.alpha=param_fwd.alpha/0.5;
            ...
            for ind_sess=1:1:num_max_session
                ...
                while ((i<num_max_trial)&&(cond))
                    ...
                    % get available action set 
                        % 여기서 myArbitrator_top.var_cogload가 뭘의미하는지 모르겠음.
                        % 이게 의미가 없으면 그냥 빼도 될거같은데 
                            switch block_condition
                                case 1
                                    map.action_valid=[[1 1]; [1 1]];        
                    ...
                    if((mode_data_prev*mode_data)<0) % if there is any visual cue change, then goal mode 
                        if(mode_data<0) % flexible to specific
                             % 불구하고....
                            myArbitrator_top.backward_flag=1; % after backward update, should set it to 0.
                        else(mode_data>0) % specific to flexible
                            myArbitrator_top.backward_flag=-1; 
                        end
                    end
     ...
     %%               
                    while ((myArbitrator_top.ind_active_model==1)&&(~map.JobComplete))     % ||((myArbitrator_top.ind_active_model==2)&&(~map.JobComplete))
                        ...
                        state_data=map.data(map.trial,3+map.index); % first state, state 2, state3이 순서대로..
                        action_chosen=map.data(map.trial,6+map.index); % action1, action2..... map.index가 3이면 RT정본데? 그냥 index가 2까지만 늘어나는건가?
                        ...
                        eval_num=log(var_exp(action_chosen)/sum(var_exp));  % This is  P(s,a)
                        ...
                        % Below part is for sarsa-first updating. no differnece from upper part. checked by sw lee.
...
%                         [myArbitrator_top, state_fwd, state_sarsa]=Bayesian_Arbitration_v4(myArbitrator_top, state_fwd, state_sarsa, map); % full bayesian 
                    ...
                    mode_data_prev=mode_data; % 매 트라이얼에서 자꾸 주어지는데 이게 대체 뭔지 모르겠네.
        ...
        ARB_SAVE.myAribtration(ll) = myArbitrator_top; % 2016 10 18
        ARB_SAVE.state_fwd(ll) = state_fwd;
        ARB_SAVE.state_sarsa(ll) = state_sarsa;
        ARB_SAVE.map(ll) = map;
...
if(mode.out==1)
    data_out=struct('NegLogLikVal',Sum_NegLogLik_val(pop_id,1),'Arbitrator',myArbitrator_top,'fwd',state_fwd,'sarsa',state_sarsa, 'map',map, 'full_arb', ARB_SAVE); % 보통의 경우 하나의 서브젝트에 대해서만 얻으니까 full subject arb는 크게 필요 없음.
end
