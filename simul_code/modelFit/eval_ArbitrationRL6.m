function [data_out] = eval_ArbitrationRL6(param_in, data_in, mode)
...
% myArbitrator=DirichletProcessNormalMix_Arbitration_Init(N_state,N_action,N_transition);
...
% myArbitrator.PE_tolerance_m1=param_in(pop_id,1); % defines threshold for zero PE [disabled in this version 2016 11 10]
% myArbitrator.PE_tolerance_m2=param_in(pop_id,2); % defines threshold for zero PE 
% myArbitrator.PE_tolerance_m2=param_in(pop_id,2); % defines the learning rate of RPE estimator [disabled in this version 2016 11 10]
...
%             param_fwd.alpha=param_fwd.alpha/0.5;
            ...
            % 이부분 mode로부터 passed된 dpnmm cluster 정보를 이용해야함. 지금있는 코드 바뀔 필요있음.
%             myArbitrator.DPNMM = mode.DPNMM; % 이부분 i th subject에 맞게 외부에서! 바꾸어 주어야할듯.
            myArbitrator_top=myArbitrator; % This may be changed from 2016 11 02 part.
