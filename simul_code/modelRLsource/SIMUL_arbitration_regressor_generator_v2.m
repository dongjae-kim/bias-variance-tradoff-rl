...
for ii=1:1:size(param_regressor_type_cue,2) % collect regressor information
    ...
    % absolue order in SPM design matrix
...
% make a regressor index matrix for 1st parametric modulations
...
if(option_optimizing_model==2)
    ...
    for ff=1:1:length(list_sbj_included)
        SBJ0{1,1}=SBJ{1,list_sbj_included(ff)};                
...
for jj2=1:1:size(SBJ,2)        % each subject
    disp(sprintf('##### generating regressors for sbj%02d/%02d #######',jj2,size(SBJ,2)));
    for kk2=1:1:size(SBJ{1,jj2}.HIST_behavior_info,2)  % each main session
        ...
        % will be fed into "matlabbatch{1}.spm.stats.fmri_spec.sess(ind_global_sess).multi"
        ...
        if(use_model_regressor_cue==1)
            for nn=1:1:length(ind_regressor_type_base{1,2}.ind_reg)
                pmod(1,ind_reg).name{1,nn}=[SBJ{1,jj2}.regressor{1,ind_regressor_type_base{1,2}.ind_reg(nn)}.name];
                pmod(1,ind_reg).poly{1,nn}=1;
                pmod(1,ind_reg).param{1,nn}=param_mat{1,2}(nn,:);
                ind_reg_abs=ind_regressor_type_base{1,2}.abs_pos_in_design_mat(nn);      list_name_for_contrast{1,ind_reg_abs}=pmod(1,ind_reg).name{1,nn};                
            end
        end
