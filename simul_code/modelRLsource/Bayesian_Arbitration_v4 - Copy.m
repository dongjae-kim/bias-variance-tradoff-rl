...
% %     myArbitrator.m2_inv_Fano=(1.0)./myArbitrator.m2_var;
    ...
    % [NOTE] we will use "PE_tolerance_m2" as the learning rate this case!!!
    lrate_estimator=myArbitrator.PE_tolerance_m2; 
    myArbitrator.m2_absPEestimate=myArbitrator.m2_absPEestimate+lrate_estimator*(abs(myState2.RPE_history(myState2.index))-myArbitrator.m2_absPEestimate);
    myArbitrator.m2_inv_Fano=[0; (40-myArbitrator.m2_absPEestimate); 0]; % [0,1]. the smaller the error estimate, the more reliable
...
input0=myArbitrator.m1_inv_Fano;    input=myArbitrator.m1_wgt'*myArbitrator.m1_inv_Fano;    % added [SW MAR 8]
...
switch myArbitrator.opt_ArbModel
    ...
    case 1 % invFano model   
        myArbitrator.transition_rate12=1-input/sum(input0);
    case 2 % mean (of 0-PE) model        
        input0=myArbitrator.m1_mean;    input=myArbitrator.m1_wgt'*myArbitrator.m1_mean;
        myArbitrator.transition_rate12=1-input/sum(input0);
    case 3 % uncetainty model        
        input0=myArbitrator.m1_var;    input=myArbitrator.m1_wgt'*myArbitrator.m1_var;
        myArbitrator.transition_rate12=input/sum(input0);
end
...
switch myArbitrator.opt_ArbModel
    case 0 % full model
        % as it is.
    case 1 % invFano model   
        myArbitrator.transition_rate21=1-input/sum(input0);
    case 2 % mean (of 0-PE) model        
        input0=myArbitrator.m2_mean;    input=myArbitrator.m2_wgt'*myArbitrator.m2_mean;
        myArbitrator.transition_rate21=1-input/sum(input0);
    case 3 % uncetainty model        
        input0=myArbitrator.m2_var;    input=myArbitrator.m2_wgt'*myArbitrator.m2_var;
        myArbitrator.transition_rate21=input/sum(input0);
end
