...
path0='\\143.248.30.94\bmlsamba\kdj\fmri_repro\repro_fmri\fmri_arbitration\modelRLsource';
...
save_path_neuroecon='\\143.248.30.94\bmlsamba\kdj\fmri_repro\repro_fmri\fmri_arbitration\regressors_contrasts\';
...
TYPE_REGRESSOR=[1 1, 1.5 1.5, 1.5 1.5, 1.5 1.5, 2 2, 1.5 1.5 1.5 1.5 1.5 1.5 1.5, 3 3 3 3 1.5 3 3 3 3 3 3]; % 1: parametric modulation (0-duration), 1.5:parmetric modulation (non-zero duration), 1.7:parametric modulation (with decision onset)  2: extra continuous parametric modulation (TR-fixed) - this will be used by "dummy" regressor. 3 : IDK
...
param_regressor_type_cue={'SPE', 'RPE','invFanoM1' 'invFanoM2' 'meanM1', 'meanM2', 'Qsarsa','Qfwd','Qarb', 'MAXinvFano12'};
...
for i=1:1:num_sbj_included %=1. process only 1 subject
    ...
    SBJ2{1,i}.name=LIST_SBJ{1,ind_sbj_included(i)};
    ...
    load(file_name_full);
    SBJ2{1,i}.HIST_block_condition_pre=HIST_block_condition;
    SBJ2{1,i}.HIST_behavior_info_pre=HIST_behavior_info;
    ...
    file_name_full=[mode.path_ext '\result_save\' file_name];
    load(file_name_full);
    SBJ2{1,i}.HIST_behavior_info=HIST_behavior_info;
    SBJ2{1,i}.HIST_behavior_info_Tag=HIST_behavior_info_Tag;
    SBJ2{1,i}.HIST_event_info=HIST_event_info;
    SBJ2{1,i}.HIST_event_info_Tag=HIST_event_info_Tag;
    SBJ2{1,i}.HIST_block_condition=HIST_block_condition;
    SBJ2{1,i}.HIST_block_condition_Tag=HIST_block_condition_Tag;
    num_tot_session=size(SBJ2{1,i}.HIST_behavior_info,2);
    
    SBJ2{1,i}.map_type=LIST_sbj_map_type(ind_sbj_included(i));
    
    % [fixing part!!! - for Oliver]
    if(strcmp(SBJ2{1,i}.name,'Oliver'))
        for mm=1:1:size(SBJ2{1,i}.HIST_event_info,2) % each session
            mat_fixing=SBJ2{1,i}.HIST_event_info{1,mm};
            ...
            SBJ2{1,i}.HIST_event_info{1,mm}=mat_fixed;
        end
    end
    ...
    for jk=1:1:size(SBJ2{1,i}.HIST_behavior_info,2)    tot_amount_earned_main_each_sbj = [tot_amount_earned_main_each_sbj; SBJ2{1,i}.HIST_behavior_info{1,jk}(end,17)]; end
...
if(option_optimizing_model==0)
    for ind_sbj=1:1:size(SBJ2,2)
        ...
        mode.out=99;
        SBJ_test=eval_ArbitrationRL_DPON2(model_BayesArb.param,SBJ_test,mode);
        % [3] Save
