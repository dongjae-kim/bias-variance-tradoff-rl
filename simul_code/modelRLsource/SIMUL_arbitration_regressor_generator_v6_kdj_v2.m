...
LIST_REGRESSOR={'SPE', 'RPE', 'Qfwd', 'Qsarsa', };
TYPE_REGRESSOR=[1 1, 1.5 1.5 ]; % 1: parametric modulation (0-duration), 1.5:parmetric modulation (non-zero duration), 1.7:parametric modulation (with decision onset)  2: extra continuous parametric modulation (TR-fixed) - this will be used by "dummy" regressor.
...
row_mat=[7 7 7 7 ]; % from which row in the SBJ{}.regressor matrix the signal needs to be extracted. e.g., uncertainty of 0 prediction error
...
param_regressor_type_cue={'SPE', 'RPE','Qsarsa','Qfwd'};
...
%     if(strcmp(param_regressor_type_cue{1,ii},'weigtM2')==1)    use_model_regressor_cue=0;    ind_chk=10;   end
    if(strcmp(param_regressor_type_cue{1,ii},'Qfwd')==1)    use_model_regressor_cue=1;    ind_chk=3;   end
    if(strcmp(param_regressor_type_cue{1,ii},'Qsarsa')==1)    use_model_regressor_cue=1;    ind_chk=4;   end
...
param_init=[0.3, 15.0, 4, 11.7,  0.15, 0.1];
param_BoundL=[0.2, 10, 0.5*param_init(3:1:4), 0.01, 0.01]; %[0.01, 3, 1.0, 2.0, 0.5, 5, 0.01, 0.01];
param_BoundU=[10/2, 40/2, 2*param_init(3:1:4), 0.5, 0.2]; %[1, 20, 6, 10, 4, 30, 0.5, 0.2];
