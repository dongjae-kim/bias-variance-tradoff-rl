...
% input: handles.X, output: handles.Y
% 'handles.X' is saved as 'maniX' in the work space.
% 'handles.Y' is saved as 'maniY' in the work space.
...
set(handles.UpdatesText,'String',updateString);
tic;
[handles.Y eigen_spectrum]= pca(handles.X,handles.d);
runTime = toc;
guidata(hObject, handles);
PlotEmbedding(hObject,eventdata,handles,0);
assignin ('base','maniX',handles.X); %sw
assignin ('base','maniY',handles.Y);
assignin ('base','maniY_eigen_spectrum',eigen_spectrum); %sw
...
% % added by sangwan : for a separate display
% hObject=figure('Name','Result');
% handles.output=hObject;
...
% [Y,D] = eigs(X*X',d,'sm',opts); % sw - smalllest eig
