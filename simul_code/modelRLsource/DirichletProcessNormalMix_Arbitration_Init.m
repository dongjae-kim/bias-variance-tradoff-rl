...
%      num_pes = [num_pes sum(data_in{1, 1}.DPNMM_MODEL.RPE(i).CLUSTER_DATA == 0)];
 end
 
%  disp(num_pes);
 [~, myArbitrator.PE_index_m1] = sort(abs(m1_mean));
 [~, myArbitrator.PE_index_m2] = sort(abs(m2_mean));
 if myArbitrator.PE_index_m2(1) == 0
 myArbitrator.PE_index_m2 = myArbitrator.PE_index_m2([2,1,3:end]);
 end
