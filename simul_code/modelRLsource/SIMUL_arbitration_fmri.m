function [output_info]=SIMUL_arbitration_fmri(EXP_NAME_IN,index_num,session_opt)

% in each sate, the goal is changing.
...
if(strcmp(session_opt,'pre')==1) % pre-session
    ffw_speed=4; % fast forward speed, 1: normal speed
end
if(strcmp(session_opt,'fmri')==1) % fmri-session
    ffw_speed=4; % fast forward speed, 1: normal speed
end
...
if(index_num==1) % create the image usage matrix if this is the first session
    ...
    HIST_behavior_info_Tag{1,3}='col3 - block condition - 1: G, 2: G'', 3:H, 4:H''';
...
cond=1;
while(cond) % temporarily disabled for test APR 21
    ...
    for trial=trial_set % each trial
        ...
        while(map_sbj.IsTerminal(state_sbj.state_history(state_sbj.index))==0)
            ...
            if(block_condition==1) % G
                ...
                current_goal_state=map_sbj.goal_condition_G(current_state,ceil(2*rand));
...
%             DrawFormattedText(wPtr, [txt_test, ', ' txt_test2], 100, 200);
            ...
            while(~decision_made)
                ...
                if(state_sbj.RT(state_sbj.index)>sec_limit_decision)
        ...
        if(isempty(HIST_behavior_info0))
            ...
            acc_rwd=HIST_behavior_info0(end,end);
        end
