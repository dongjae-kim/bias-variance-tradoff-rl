...
LIST_sbj_map_type=[1*ones(1,12) 2*ones(1,12)]; %1:'sangwan2012b', 2:'sangwan2012c'

% regressor list
% [CAUTION] DO NOT change the order!!!
LIST_REGRESSOR={'SPE', 'RPE', 'uncertaintyM1', 'uncertaintyM2', 'meanM1', 'meanM2', 'invFanoM1', 'invFanoM2', 'weigtM1', 'weigtM2', 'Qfwd', 'Qsarsa', 'Qarb'};
TYPE_REGRESSOR=[1 1, 2 2, 2 2, 2 2, 2 2, 1.5 1.5 1.5]; % 1: parametric modulation (0-duration), 1.5:parmetric modulation (non-zero duration), 2: extra continuous parametric modulation (TR-fixed) - this will be used by "dummy" regressor.
row_mat=[7 7 8 8 8 8 8 8 7 7 7 7 7]; % from which row in the SBJ{}.regressor matrix the signal needs to be extracted. e.g., uncertainty of 0 prediction error
...
mode.path_ext=path0;
mode.total_simul=10; % # of total simulation repetition per subject
...
param_regressor_type_cue={'SPE', 'RPE', 'Qfwd', 'Qsarsa', 'uncertaintyM1', 'invFanoM1', 'uncertaintyM2', 'invFanoM2'}; % if you change the first four regressors, you should change the reading part of SBJ.regressor accordingly.
param_regressor_type_cue_abs_pos_in_design_mat=[[2 3] [5 6] [7 8 9 10]]; % absolute position in design matirx - should reserve the baseline regressors. [NOTE!!!]  should always make ascending order.
...
for ii=1:1:size(param_regressor_type_cue,2) % collect regressor information
    ...
    % absolue column index in SPM design matrix
    ind_regressor_total_in_design_mat=[ind_regressor_total_in_design_mat, param_regressor_type_cue_abs_pos_in_design_mat(ii)];
    % regressor type
    type_regressor=[type_regressor TYPE_REGRESSOR(ind_chk)];    
end
% make a regressor index matrix for 1st parametric modulations (normal)
ind_regressor_type_base{1,1}.ind_reg=ind_regressor_total(find(type_regressor==1));
ind_regressor_type_base{1,1}.abs_pos_in_design_mat=ind_regressor_total_in_design_mat(find(type_regressor==1));
ind_regressor_type_base{1,2}.ind_reg=ind_regressor_total(find(type_regressor==1.5));
ind_regressor_type_base{1,2}.abs_pos_in_design_mat=ind_regressor_total_in_design_mat(find(type_regressor==1.5));
...
ind_regressor_type_dummy.abs_pos_in_design_mat=ind_regressor_total_in_design_mat(find(type_regressor==2));
...
param_BoundL=[0.1,10/2, 0.5*param_init(3:1:6), 0.01, 0.01]; %[0.01, 3, 1.0, 2.0, 0.5, 5, 0.01, 0.01];
...
for jj2=1:1:size(SBJ,2)        % each subject
    disp(sprintf('##### writing regressors for sbj%02d/%02d #######',jj2,size(SBJ,2)));
    for kk2=1:1:size(SBJ{1,jj2}.HIST_behavior_info,2)  % each main session
        ...
        for ll2=1:1:length(col_on)
            ...
            if(use_model_regressor_cue==1)
                ...
                for nn=1:1:length(ind_regressor_type_base{1,1}.ind_reg)
                    ...
                    if(mytrial_s==1) % the first decision stage
                        param_mat{1,1}(nn,ll2)=0.0; % [NOTE] NO signal at the first stage for the case of SPE/RPE
                ...
                % regressor type2 : for Qfwd/Qsarsa
                for nn=1:1:length(ind_regressor_type_base{1,2}.ind_reg)
                    mysession=kk2;
                    myblock=mat_work(1,col_on(ll2)); % block in session
                    mytrial=mat_work(2,col_on(ll2)); % trial in block
                    mytrial_s=mat_work(3,col_on(ll2)); % trial_s in trial (arbitration index: 1 at the second stage, 2 at the third stage)                                        
                    if(mytrial_s==3) % the third decision stage
                        param_mat{1,2}(nn,ll2)=0.0; % no Q-value for the last presentation because there is no action.
                    else % the first and the second decision stage
        ...
        % 1st regressors
        ind_reg=ind_reg+1;
        durations{1,ind_reg}=0;
        onsets{1,ind_reg}=onset_mat;
        names{1,ind_reg}=['Cue_{0T}'];
        ...
        onsets{1,ind_reg}=onset_mat;
        names{1,ind_reg}=['Cue_{1T}'];
        ind_reg_abs=4;      list_name_for_contrast{1,ind_reg_abs}=names{1,ind_reg}; % add to the global list of regresssors
        ...
        if(use_model_regressor_cue==1)
            ...
            for nn=1:1:length(ind_regressor_type_dummy.ind_reg)                
                ...
                pmod(1,ind_reg).poly{1,ind_reg_param}=1;
                pmod(1,ind_reg).param{1,ind_reg_param}=rand(1,length(param_mat{1,2}(1,:)));
...
eval(['save ' save_path_result 'contrast_spm.mat' ' contrast_spm ind_regressor_type_base ind_regressor_type_dummy'])
eval(['save ' save_for_SPM 'contrast_spm.mat' ' contrast_spm ind_regressor_type_base ind_regressor_type_dummy'])
eval(['save ' save_path_neuroecon 'contrast_spm.mat' ' contrast_spm ind_regressor_type_base ind_regressor_type_dummy'])
